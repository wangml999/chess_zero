...
class Network(object):
    def __init__(self, name, sess, N, channels, training=False, logdir=None):
        ...
        with tf.variable_scope(name):
            ...
            resnet = tf.layers.batch_normalization(resnet, name="pre_bn1", training=self.training)
            ...
            for block in range(num_blocks):
                ...
                with tf.variable_scope("res_block_{0}".format(block)):
                    ...
                    resnet = tf.layers.batch_normalization(resnet, name="bn1", training=self.training)
                    ...
                    if use_separatable_conv:
                        ...
                        resnet = tf.layers.conv2d(resnet,
                                               ...
                                               kernel_regularizer=regularizer,
                                               kernel_initializer=conv_init)
                    resnet = tf.layers.batch_normalization(resnet, name="bn2", training=self.training)
            ...
            with tf.variable_scope("policy_head"):
                ...
                policy_net = tf.layers.batch_normalization(policy_net, name="bn", training=self.training)
                ...
                #self.action_prob = tf.clip_by_value(self.action_prob, 0, 1 - 1e-7, name="out_action_prob")
            ...
            with tf.variable_scope("value_head"):
                value_net = tf.layers.conv2d(resnet,
                                       ...
                                       kernel_regularizer=regularizer,
                                       kernel_initializer=conv_init)
                value_net = tf.layers.batch_normalization(value_net, name="bn", training=self.training)
        ...
        if training:
            with tf.variable_scope("action_loss"):
                ...
                #self.actor_loss = -tf.reduce_mean(tf.reduce_sum(self.actions_pi * tf.log(tf.clip_by_value(self.action_prob, 1e-10,1.0)), keep_dims=True, axis=1))
            ...
            with tf.variable_scope("value_loss"):
                ...
                #self.value_loss = tf.losses.mean_squared_error(value_net, self.rewards)
            ...
            with tf.variable_scope("total_loss"):
                ...
                self.total_loss = self.actor_loss * 0.1 + self.value_loss + self.l2_loss
                #self.total_loss = self.value_loss - 0.1*self.l2_loss
            ...
            if logdir:
                ...
                logits_summary = tf.summary.histogram("logits", self.logits)
                #ratio_hist_summary = tf.summary.histogram("ratio", ratio)

                self.summary_op = tf.summary.merge([action_summary, value_loss_summary, entry_loss_summary, loss_summary, value_summary, logits_summary, learning_rate_summary])
